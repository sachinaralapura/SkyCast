...
export default function DefaultScreen({ currentWeatherData, forecastLocation, onHandleClick }) {
  ...
  const fetchSuggestions = async function (label) {
    const response = await fetch(`https://nominatim.openstreetmap.org/search?q=${label}&format=jsonv2`);
            ...
            <>
              ...
              <div className="temp-container">
                ...
                <div>
                  <p style={{ fontSize: "144px" }}>
                    {parseFloat(currentWeatherData[0]?.values?.temperature2m || 0).toFixed(0)}
                  </p>
                  <p className="text-capitalize">{currentWeatherData[0]?.values?.weatherCondition || "N/A"}</p>
                </div>
                <p
                  style={{
                    fontSize: "24px",
              ...
              >
                ...
                <div className="weather-info-subtitle">
                  ...
                  <p>{parseFloat(currentWeatherData[0]?.values?.windSpeed || 0).toFixed(0)} km/h</p>
                </div>
              </div>
            </>
          )}
            ...
            <div className="search-city-suggestions">
              {suggestions?.length > 0 &&
                suggestions.map((suggestion, suggestionIndex) => (
                  <p className="suggested-label" key={suggestionIndex} onClick={() => onHandleClick(suggestion)}>
                    {suggestion.label}
                  </p>
                ))}
            </div>
